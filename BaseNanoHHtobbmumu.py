...
class BaseNanoHHtobbmumu(NanoAODHistoModule):
    ...
    def prepareTree(self, tree, sample=None, sampleCfg=None, backend=None):
        ...
        if not self.is_MC:
            JECTagDatabase = {
                              "HIPM_UL2016": "Summer19UL16_RunBCDEF_V7_DATA", 
                              "-UL2016": "Summer19UL16_RunFGH_V7_DATA", 
    ...
    def definePlots(self, tree, noSel, sample=None, sampleCfg=None):
        ...
        MuMuLooseSel = op.combine(self.cleanedMuons, N=2, pred=lambda l1,l2 : l1.charge != l2.charge)
        ElElLooseSel = op.combine(self.cleanedElectrons, N=2, pred=lambda l1,l2 : l1.charge != l2.charge)
        ElMuLooseSel = op.combine((self.cleanedElectrons, self.cleanedMuons), N=2, pred=lambda l1,l2 : l1.charge != l2.charge)
        
        def ptCutElEl(dilep,l1,l2,era): return op.AND(
            elConePt(dilep)[l1.idx] > 20.,
            elConePt(dilep)[l2.idx] > 20.,
            op.OR(elConePt(dilep)[l1.idx] > 29., elConePt(dilep)[l2.idx] > 29.) if era == "2017" else op.OR(elConePt(dilep)[l1.idx] > 26., elConePt(dilep)[l2.idx] > 26.)
            )

        def ptCutMuMu(dilep,l1,l2,era): return op.AND(
            muonConePt(dilep)[l1.idx] > 20.,
            muonConePt(dilep)[l2.idx] > 20.,
            op.OR(muonConePt(dilep)[l1.idx] > 29., muonConePt(dilep)[l2.idx] > 29.) if era == "2017" else op.OR(muonConePt(dilep)[l1.idx] > 26., muonConePt(dilep)[l2.idx] > 26.)
            )

        def ptCutElMu(dilepEl,dilepMu,l1,l2,era): return op.AND(
            elConePt(dilepEl)[l1.idx] > 20.,
            muonConePt(dilepMu)[l2.idx] > 20.,
            op.OR(elConePt(dilepEl)[l1.idx] > 29., muonConePt(dilepMu)[l2.idx] > 29.) if era == "2017" else op.OR(elConePt(dilepEl)[l1.idx] > 26., muonConePt(dilepMu)[l2.idx] > 26.)
            )
        # OS tight dilepton collections
        ElElTightSel = op.combine(self.tightElectrons, N=2, pred=lambda l1, l2 : op.AND(l1.charge != l2.charge, op.OR(ptCutElEl(self.tightElectrons,l1,l2,era) )))
        MuMuTightSel = op.combine(self.tightMuons, N=2, pred=lambda l1, l2 : op.AND(l1.charge != l2.charge, op.OR(ptCutMuMu(self.tightMuons,l1,l2,era) )))
        ElMuTightSel = op.combine((self.tightElectrons, self.tightMuons), N=2, pred=lambda l1,l2 : op.AND(l1.charge != l2.charge, op.OR(ptCutElMu(self.tightElectrons,self.tightMuons,l1,l2,era) )))
        ...
        hasZmm      = noSel.refine("hasZmm",  cut=[ (tree.PV).npvsGood > 0 , mllCut, leptonMultiplicityCut_mumu,
                                                   op.rng_len(cleanedHighestBtagScoreJets) > 1,
                                                   op.NOT(outZ(bestZmm)),
                                                   out_of_range_mass(cleanedHighestBtagScoreJets,min_mass_bjets,max_mass_bjets)])
        
        hasHmm      = noSel.refine("hasHmm",  cut=[ (tree.PV).npvsGood > 0 , mllCut, leptonMultiplicityCut_mumu,
                                                   op.rng_len(cleanedHighestBtagScoreJets) > 1,
                                                   op.NOT(outH(bestHmm)),
                                                   out_of_range_mass(cleanedHighestBtagScoreJets,min_mass_bjets,max_mass_bjets) ])
        hasZjets    = noSel.refine("hasZjets",  cut=[ (tree.PV).npvsGood > 0 ,mllCut, leptonMultiplicityCut_mumu,
                                                   op.rng_len(cleanedHighestBtagScoreJets) > 1,
                                                   outZ(bestZmm),
                                                   op.NOT(out_of_range_mass(cleanedHighestBtagScoreJets,min_mass_bjets,max_mass_bjets)) ])
        
        hasHjets    = noSel.refine("hasHjets",  cut=[ (tree.PV).npvsGood > 0 ,mllCut, leptonMultiplicityCut_mumu,
                                                   op.rng_len(cleanedHighestBtagScoreJets) > 1,
                                                   op.NOT(outH(bestHmm)),
                                                   out_of_range_mass(cleanedHighestBtagScoreJets,min_mass_bjets,max_mass_bjets) ])
        isSideband2lbbZmm = noSel.refine("isSideband2lbbZmm", cut=[ (tree.PV).npvsGood > 0 ,mllCut, leptonMultiplicityCut_mumu,
                                                   op.rng_len(cleanedHighestBtagScoreJets) > 1,
                                                   outZ(bestZmm),
                                                   out_of_range_mass(cleanedHighestBtagScoreJets,min_mass_bjets,max_mass_bjets)])
        isSideband2lbbHmm = noSel.refine("isSideband2lbbHmm",  cut=[ (tree.PV).npvsGood > 0 ,mllCut,leptonMultiplicityCut_mumu,
                                                   op.rng_len(cleanedHighestBtagScoreJets) > 1,
                                                   outH(bestHmm),
                                                   out_of_range_mass(cleanedHighestBtagScoreJets,min_mass_bjets,max_mass_bjets) ])
        isSignalRegion = noSel.refine("isSignalRegion", 
                                                   cut=[
                                                   (tree.PV).npvsGood > 0 ,mllCut, leptonMultiplicityCut_mumu,
                                                   op.rng_len(cleanedHighestBtagScoreJets) > 1,
                                                   op.NOT(outH(bestHmm)),
                                                   op.NOT(out_of_range_mass(cleanedHighestBtagScoreJets,min_mass_bjets,max_mass_bjets)) ])
    ...
    def plotPairs(self, pair, sel, category, muons):
            ...
            if "SignalRegion" in category:
                ...
                plots.append(Plot.make1D(f"h_{category}_mll", op.invariant_mass(pair[0].p4, pair[1].p4), sel, EquidistantBinning(100, 20., 200.), title="Invariant Mass of two muons", xTitle= "m_{\mu\mu} (GeV)",plotopts={'no-data': True, 'show-ratio': False}))
                ...
                plots.append(Plot.make1D(f"h_{category}_lep2_pT", pair[1].p4.Pt(), sel, EquidistantBinning(50, 20., 200.), title=" lepton2 pT", xTitle= "pT (GeV)",plotopts={'no-data': True, 'show-ratio': False}))
